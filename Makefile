...
files: create-dirs 
	install -m ${MODE} image/init.d/checkfs       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/cleanfs       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/halt          ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/console       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/localnet      ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/modules       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/mountfs       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/mountvirtfs   ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/network       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/rc            ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/reboot        ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/sendsignals   ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/setclock      ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/swap          ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/sysctl        ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/sysklogd      ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/template      ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/udev          ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/init.d/udev_retry    ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} image/sbin/ifup            ${SBIN}
	install -m ${MODE} image/sbin/ifdown          ${SBIN}
	install -m ${CONFMODE} image/sbin/ifup.8      ${MAN8}
	ln -sf  ifup.8                                ${MAN8}/ifdown.8
	install -m ${MODE}     image/lib/services/ipv4-static-route  ${SERVICEDIR}
	install -m ${MODE}     image/lib/services/ipv4-static        ${SERVICEDIR}
	install -m ${CONFMODE} image/lib/services/init-functions     ${SERVICEDIR}

	if [ ! -f ${ETCDIR}/sysconfig/createfiles ]; then \
          install -m ${CONFMODE} image/sysconfig/createfiles ${ETCDIR}/sysconfig/ ;\
        fi
	if [ ! -f ${ETCDIR}/sysconfig/modules     ]; then \
          install -m ${CONFMODE} image/sysconfig/modules     ${ETCDIR}/sysconfig/ ;\
        fi
	if [ ! -f ${ETCDIR}/sysconfig/udev_retry  ]; then \
          install -m ${CONFMODE} image/sysconfig/udev_retry  ${ETCDIR}/sysconfig/ ;\
        fi
	if [ ! -f ${ETCDIR}/sysconfig/rc.site     ]; then \
          install -m ${CONFMODE} image/sysconfig/rc.site     ${ETCDIR}/sysconfig/ ;\
        fi
